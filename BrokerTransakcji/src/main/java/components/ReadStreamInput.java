...
public class ReadStreamInput {
    public CommandData readStream(String[] args) throws IOException, ParseException {
        ...
        if (cmd.hasOption("broker"))
            commandData.setBroker(cmd.getOptionValue("broker"));
        if (cmd.hasOption("queue"))
            commandData.setQueue(cmd.getOptionValue("queue"));
        if (cmd.hasOption("topic"))
            commandData.setTopic(cmd.getOptionValue("topic"));
    ...
    private Options createOptions(){
        ...
        options.addOption("broker", "broker", true, "Broker");
        options.addOption("queue", "queue", true, "Kolejka");
        options.addOption("topic", "topic", true, "Temat");
        return options;
    }
}
